...
def generate_api_doc():
    ...
    for module_index, module_name in enumerate(API_ROUTES, start=1):
        content.append(f'{module_index}. [{module_name}](#{module_name.replace(" ", "-")})')
    content.append('')
    ...
    for module_index, module_name in enumerate(API_ROUTES, start=1):
        ...
        # 创建表格
        content.append('| API路径 | 方法 | 功能描述 | 是否被使用 |')
        content.append('|---------|------|---------|-----------|')
        
        # 填充表格数据
        for route in routes:
            content.append(f'| {route[0]} | {route[1]} | {route[2]} | {route[3]} |')
    ...
    # 保存文档
    doc_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'api_document.md')
